...
import { dentalDashboardData } from "@/data/seeded/dental";

function getChairStatusClasses(status: string): string {
...
const sidebarItems = [
  ...
  { icon: Users, label: "Chair Queue", badge: "48" },
  { icon: Scissors, label: "Surgery List", badge: "4" },
  { icon: Sparkles, label: "Orthodontics", badge: "12" },
  { icon: Clock, label: "Appointments" },
  { icon: Activity, label: "Procedures" },
...
export function DentalDashboard() {
  ...
  const data = dentalDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/dental/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Dental</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Pooja Mehta</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Dental Surgeon</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Chair Queue</h3>
                <span className="text-[10px] text-blue-600 font-medium">{data.chairQueue.length} patients</span>
              </div>
              <div className="space-y-2">
                {data.chairQueue.map((patient) => (
                  <div key={patient.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-500">{patient.chair} • {patient.time}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${getChairStatusClasses(patient.status)}`}>
                        {patient.status}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Procedure Stats (Total)</h3>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <div className="p-3 rounded-lg bg-blue-50">
                  <p className="text-xs font-medium text-gray-700 mb-1">Restorative</p>
                  <p className="text-xl font-bold text-blue-700">{data.procedureBreakdown.restorative.total}</p>
                  <p className="text-[10px] text-gray-500">RCT: {data.procedureBreakdown.restorative.rct}</p>
                </div>
                <div className="p-3 rounded-lg bg-purple-50">
                  <p className="text-xs font-medium text-gray-700 mb-1">Prosthetics</p>
                  <p className="text-xl font-bold text-purple-700">{data.procedureBreakdown.prosthetics.total}</p>
                  <p className="text-[10px] text-gray-500">Crowns: {data.procedureBreakdown.prosthetics.crowns}</p>
                </div>
                <div className="p-3 rounded-lg bg-green-50">
                  <p className="text-xs font-medium text-gray-700 mb-1">Periodontal</p>
                  <p className="text-xl font-bold text-green-700">{data.procedureBreakdown.periodontal.total}</p>
                  <p className="text-[10px] text-gray-500">Scaling: {data.procedureBreakdown.periodontal.scaling}</p>
                </div>
                <div className="p-3 rounded-lg bg-red-50">
                  <p className="text-xs font-medium text-gray-700 mb-1">Oral Surgery</p>
                  <p className="text-xl font-bold text-red-700">{data.procedureBreakdown.oral_surgery.total}</p>
                  <p className="text-[10px] text-gray-500">Implants: {data.procedureBreakdown.oral_surgery.implant}</p>
                </div>
                <div className="p-3 rounded-lg bg-pink-50">
                  <p className="text-xs font-medium text-gray-700 mb-1">Orthodontics</p>
                  <p className="text-xl font-bold text-pink-700">{data.procedureBreakdown.orthodontics.total}</p>
                  <p className="text-[10px] text-gray-500">Active: {data.orthoRegistry.active}</p>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly Patients & Procedures"

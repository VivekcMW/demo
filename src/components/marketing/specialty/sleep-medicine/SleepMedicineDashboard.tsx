...
import { sleepMedicineDashboardData } from "@/data/seeded/sleep-medicine";

function getStatusClasses(status: string): string {
  if (status === "in-consult") return "bg-green-100 text-green-700";
  if (status === "in-study") return "bg-purple-100 text-purple-700";
...
const sidebarItems = [
  ...
  { icon: BedDouble, label: "Sleep Studies", badge: "6" },
  { icon: Waves, label: "PSG Lab" },
  { icon: Clock, label: "CPAP Clinic" },
...
export function SleepMedicineDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = sleepMedicineDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/sleep-medicine/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Sleep Medicine</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Nikhil Desai</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Sleep Specialist</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <Stat
                title="Sleep Studies"
                value={data.todayStats.sleepStudies}
                icon={<BedDouble className="w-4 h-4 text-purple-500" />}
              />
              <Stat
                title="CPAP Titrations"
                value={data.todayStats.cpapTitrations}
              ...
              <div className="space-y-2">
                {data.patientQueue.map((patient) => (
                  <div key={patient.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${patient.ahi && patient.ahi >= 30 ? "bg-red-100 text-red-700" : patient.ahi && patient.ahi >= 15 ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>
                        {patient.ahi ? `AHI ${patient.ahi}` : "New"}
                      </div>
                      <div>
                        <p className="text-xs font-medium text-gray-900">{patient.name}</p>
                        <p className="text-[10px] text-gray-500">{patient.complaint}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Sleep Lab</h3>
                  <span className="text-[10px] text-gray-500">{data.labStatus.occupied}/{data.labStatus.totalBeds} beds</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-indigo-50 text-center">
                    <p className="text-lg font-bold text-indigo-700">{data.labStatus.occupied}</p>
                    <p className="text-[10px] text-gray-500">Tonight</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.labStatus.pending}</p>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Active Registry</h3>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-red-50">
                    <span className="text-xs font-medium text-gray-700">Severe OSA</span>
                    <span className="text-sm font-bold text-red-700">{data.diagnosisBreakdown.severeOSA}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-orange-50">
                    <span className="text-xs font-medium text-gray-700">Moderate OSA</span>
                    <span className="text-sm font-bold text-orange-700">{data.diagnosisBreakdown.moderateOSA}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">Insomnia</span>
                    <span className="text-sm font-bold text-purple-700">{data.diagnosisBreakdown.insomnia}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly OPD & Sleep Studies"
              data={data.weeklyTrend}
              primaryLabel="OPD"
              secondaryLabel="Studies"
            />
          </div>
        </div>
      </div>

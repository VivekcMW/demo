...
import { ivfDashboardData } from "@/data/seeded/ivf";

function getCycleStatusClasses(status: string): string {
  if (status === "in-stim") return "bg-purple-100 text-purple-700";
  if (status === "ready-et") return "bg-green-100 text-green-700";
  if (status === "trigger") return "bg-amber-100 text-amber-700";
  return "bg-blue-100 text-blue-700";
}

const sidebarItems = [
  { icon: LayoutDashboard, label: "Dashboard", active: true },
  { icon: Users, label: "Consultations", badge: "22" },
  { icon: Sparkles, label: "Active Cycles", badge: "65" },
  { icon: FlaskConical, label: "Embryology Lab" },
  { icon: Baby, label: "Outcomes" },
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/ivf/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">IVF & Fertility</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Sneha Malhotra</p>
                  <p className="text-[9px] text-gray-500 leading-tight">IVF Specialist</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              />
              <Stat
                title="Egg Retrievals"
                value={data.todayStats.eggRetrievals}
                icon={<Sparkles className="w-4 h-4 text-pink-500" />}
              />
              <Stat
                title="Embryo Transfers"
                value={data.todayStats.embryoTransfers}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Active Cycles</h3>
                <span className="text-[10px] text-purple-600 font-medium">{data.cycleQueue.length} patients</span>
              </div>
              <div className="space-y-2">
                {data.cycleQueue.map((patient) => (
                  <div key={patient.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">{patient.cycle} • {patient.day}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-500">{patient.time}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${getCycleStatusClasses(patient.status)}`}>
                        {patient.status}
                      </span>
                    </div>
                  </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Cycle Statistics</h3>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <div>
                      <span className="text-xs font-medium text-gray-700">IVF-ICSI</span>
                      <p className="text-[10px] text-gray-500">Success: {data.cycleStats.ivfIcsi.successRate}%</p>
                    </div>
                    <span className="text-sm font-bold text-purple-700">{data.cycleStats.ivfIcsi.active}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-pink-50">
                    <div>
                      <span className="text-xs font-medium text-gray-700">FET</span>
                      <p className="text-[10px] text-gray-500">Success: {data.cycleStats.fet.successRate}%</p>
                    </div>
                    <span className="text-sm font-bold text-pink-700">{data.cycleStats.fet.active}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <div>
                      <span className="text-xs font-medium text-gray-700">IUI</span>
                      <p className="text-[10px] text-gray-500">Success: {data.cycleStats.iui.successRate}%</p>
                    </div>
                    <span className="text-sm font-bold text-blue-700">{data.cycleStats.iui.active}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Embryology Lab</h3>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.labStatus.embryosCultured}</p>
                    <p className="text-[10px] text-gray-500">Culturing</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.labStatus.frozenEmbryos}</p>
                    <p className="text-[10px] text-gray-500">Frozen</p>
                  </div>
                  <div className="p-2 rounded-lg bg-pink-50 text-center">
                    <p className="text-lg font-bold text-pink-700">{data.labStatus.oocytesBanked}</p>
                    <p className="text-[10px] text-gray-500">Oocytes</p>
                  </div>
                  <div className="p-2 rounded-lg bg-green-50 text-center">
                    <p className="text-lg font-bold text-green-700">{data.labStatus.spermSamples}</p>
                    <p className="text-[10px] text-gray-500">Sperm</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly Consultations & Procedures"
              data={data.weeklyTrend}
              primaryLabel="Consults"

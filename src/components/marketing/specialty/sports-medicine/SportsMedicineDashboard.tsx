...
import { sportsMedicineDashboardData } from "@/data/seeded/sports-medicine";

function getStatusClasses(status: string): string {
  if (status === "in-consult") return "bg-green-100 text-green-700";
  if (status === "in-therapy") return "bg-purple-100 text-purple-700";
...
const sidebarItems = [
  ...
  { icon: Target, label: "Injuries", badge: "45" },
  { icon: Zap, label: "Performance" },
  { icon: Timer, label: "Rehab" },
...
export function SportsMedicineDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = sportsMedicineDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/sports-medicine/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Sports Medicine</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Rahul Malik</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Sports Medicine</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <Stat
                title="Injury Evals"
                value={data.todayStats.injuryEvals}
                icon={<Target className="w-4 h-4 text-red-500" />}
              />
              <Stat
                title="Performance"
                value={data.todayStats.performanceTests}
                icon={<Zap className="w-4 h-4 text-yellow-500" />}
              />
              <Stat
                title="Rehab Sessions"
                value={data.todayStats.rehabSessions}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Athlete Queue</h3>
              </div>
              <div className="space-y-2">
                {data.athleteQueue.map((athlete) => (
                  <div key={athlete.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div>
                      <p className="text-xs font-medium text-gray-900">{athlete.name}</p>
                      <p className="text-[10px] text-gray-500">{athlete.sport} • {athlete.complaint}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-500">{athlete.time}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${getStatusClasses(athlete.status)}`}>
                        {athlete.status}
                      </span>
                    </div>
                  </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Injury Registry</h3>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-red-50 text-center">
                    <p className="text-lg font-bold text-red-700">{data.injuryRegistry.acl}</p>
                    <p className="text-[10px] text-gray-500">ACL</p>
                  </div>
                  <div className="p-2 rounded-lg bg-orange-50 text-center">
                    <p className="text-lg font-bold text-orange-700">{data.injuryRegistry.rotatorCuff}</p>
                    <p className="text-[10px] text-gray-500">Rotator Cuff</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.injuryRegistry.ankle}</p>
                    <p className="text-[10px] text-gray-500">Ankle</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.injuryRegistry.concussion}</p>
                    <p className="text-[10px] text-gray-500">Concussion</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-green-50">
                    <span className="text-xs font-medium text-gray-700">Return to Sport</span>
                    <span className="text-sm font-bold text-green-700">{data.outcomes.returnToSport}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">Full Recovery</span>
                    <span className="text-sm font-bold text-blue-700">{data.outcomes.fullRecovery}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">Re-injury Rate</span>
                    <span className="text-sm font-bold text-purple-700">{data.outcomes.reinjuryRate}%</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly OPD & Rehab Sessions"
              data={data.weeklyTrend}
              primaryLabel="OPD"
              secondaryLabel="Rehab"
            />
          </div>
        </div>
      </div>

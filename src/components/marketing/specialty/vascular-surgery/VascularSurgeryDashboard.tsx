...
import { vascularSurgeryDashboardData } from "@/data/seeded/vascular-surgery";

function getStatusClasses(status: string): string {
  if (status === "in-surgery") return "bg-green-100 text-green-700";
  if (status === "in-cath-lab") return "bg-purple-100 text-purple-700";
...
const sidebarItems = [
  ...
  { icon: GitBranch, label: "Surgeries", badge: "6" },
  { icon: Zap, label: "Endovascular", badge: "4" },
  { icon: Heart, label: "Vein Clinic" },
...
export function VascularSurgeryDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = vascularSurgeryDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/vascular-surgery/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Vascular Surgery</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Manoj Rao</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Vascular Surgeon</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <Stat
                ...
                value={data.todayStats.openSurgeries}
                icon={<GitBranch className="w-4 h-4 text-red-500" />}
              />
              <Stat
                title="Endovascular"
                value={data.todayStats.endovascular}
                icon={<Zap className="w-4 h-4 text-purple-500" />}
              />
              <Stat
                title="Wound Care"
                value={data.todayStats.woundCare}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-blue-600 font-medium">{data.surgerySchedule.length} cases</span>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-red-50 text-center">
                    <p className="text-lg font-bold text-red-700">{data.procedureBreakdown.aortic.active}</p>
                    <p className="text-[10px] text-gray-500">Aortic</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.procedureBreakdown.peripheral.active}</p>
                    <p className="text-[10px] text-gray-500">Peripheral</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.procedureBreakdown.venous.active}</p>
                    <p className="text-[10px] text-gray-500">Venous</p>
                  </div>
                  <div className="p-2 rounded-lg bg-green-50 text-center">
                    <p className="text-lg font-bold text-green-700">{data.procedureBreakdown.dialysisAccess.active}</p>
                    <p className="text-[10px] text-gray-500">Dialysis Access</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-green-50">
                    <span className="text-xs font-medium text-gray-700">Patency Rate</span>
                    <span className="text-sm font-bold text-green-700">{data.outcomes.patencyRate}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">Limb Salvage</span>
                    <span className="text-sm font-bold text-blue-700">{data.outcomes.limbSalvage}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">30-Day Mortality</span>
                    <span className="text-sm font-bold text-purple-700">{data.outcomes.mortality30Day}%</span>
                  </div>
                </div>
              </div>
            </div>

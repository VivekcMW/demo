...
import { orthopedicsDashboardData } from "@/data/seeded/orthopedics";
...
function getFractureStatusClasses(status: string): string {
  if (status === "healing") return "bg-green-100 text-green-700";
  if (status === "pre-op") return "bg-amber-100 text-amber-700";
  if (status === "post-op") return "bg-blue-100 text-blue-700";
  return "bg-gray-100 text-gray-600";
}

function getStockStatusClasses(status: string): string {
...
const sidebarItems = [
  ...
  { icon: Users, label: "OPD Queue", badge: "5" },
  { icon: Scissors, label: "OT Schedule", badge: "6" },
  { icon: Bone, label: "Fracture Census", badge: "45" },
  { icon: Package, label: "Implants" },
  { icon: Dumbbell, label: "Physio", badge: "22" },
...
export function OrthopedicsDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = orthopedicsDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/orthopedics/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Orthopedics</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Vikram Sharma</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Ortho Surgeon</p>
                </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50">
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-sm font-semibold text-gray-900">Orthopedics Dashboard</h1>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-medium">
                    ...
                    {data.implantInventory.lowStock} low stock
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
                ...
                <Stat
                  title="Fractures"
                  value={data.todayStats.fractureCases}
                  icon={<Bone className="w-3.5 h-3.5 text-orange-600" />}
                />
                <Stat
                  title="New"
                ...
                <Stat
                  title="Physio"
                  value={data.todayStats.physioSessions}
                  icon={<Dumbbell className="w-3.5 h-3.5 text-teal-600" />}
                />
                <Stat
                  title="Cast Off"
                  value={data.todayStats.castRemovals}
                  icon={<Bone className="w-3.5 h-3.5 text-gray-600" />}
                />
                <Stat
                  title="Revenue"
                  value={`₹${(data.todayStats.revenue / 100000).toFixed(1)}L`}
                  icon={<IndianRupee className="w-3.5 h-3.5 text-indigo-600" />}
                />
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-3 rounded-lg bg-white border border-gray-200">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button type="button" className="text-[10px] text-indigo-600 hover:underline flex items-center gap-0.5">
                        View all <ChevronRight className="w-3 h-3" />
                      </button>
                    </div>
                    <div className="space-y-1.5">
                      {data.otSchedule.surgeries.map((surgery) => (
                        <div
                          key={surgery.id}
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between text-[10px] text-gray-600">
                            <span className="truncate max-w-40">{surgery.procedure}</span>
                    ...
                    <div className="space-y-1.5">
                      {data.currentQueue.slice(0, 3).map((patient) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              <span className="w-6 h-6 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[10px] font-bold">
                                {patient.tokenNo}
                              </span>
                              <div>
                            ...
                            <div className="flex items-center gap-1.5">
                              {patient.xrayRequired && (
                                <span className="px-1.5 py-0.5 text-[9px] rounded bg-purple-100 text-purple-700">X-ray</span>
                              )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-3 rounded-lg bg-white border border-gray-200">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-semibold text-gray-900 flex items-center gap-1.5">
                        ...
                        Fracture Census
                        <span className="ml-1 px-1.5 py-0.5 text-[9px] rounded bg-orange-100 text-orange-700">
                          {data.fractureCensus.activeCases} active
                        </span>
                      </h4>
                      <button type="button" className="text-[10px] text-indigo-600 hover:underline flex items-center gap-0.5">
                        View all <ChevronRight className="w-3 h-3" />
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-2">
                      <div className="text-center p-1.5 rounded bg-blue-50">
                        <p className="text-sm font-bold text-blue-700">{data.fractureCensus.upperLimb}</p>
                        <p className="text-[8px] text-blue-600">Upper Limb</p>
                      </div>
                      <div className="text-center p-1.5 rounded bg-green-50">
                        <p className="text-sm font-bold text-green-700">{data.fractureCensus.lowerLimb}</p>
                        <p className="text-[8px] text-green-600">Lower Limb</p>
                      </div>
                      <div className="text-center p-1.5 rounded bg-purple-50">
                        <p className="text-sm font-bold text-purple-700">{data.fractureCensus.spine}</p>
                        <p className="text-[8px] text-purple-600">Spine</p>
                      </div>
                    </div>

                    {/* Recent Cases */}
                    <div className="space-y-1">
                      {data.fractureCensus.recentCases.map((fx) => (
                        <div key={fx.id} className="flex items-center justify-between py-1 px-2 rounded bg-gray-50 text-[10px]">
                          <div className="flex-1">
                            <span className="text-gray-700 font-medium">{fx.name}</span>
                            <span className="text-gray-400 ml-1">· {fx.site}</span>
                          </div>
                          <span className={`px-1.5 py-0.5 rounded ${getFractureStatusClasses(fx.status)}`}>
                            {fx.status}
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="p-3 rounded-lg bg-white border border-gray-200">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-semibold text-gray-900 flex items-center gap-1.5">
                        ...
                        Implant Inventory
                        {data.implantInventory.lowStock > 0 && (
                          <span className="ml-1 px-1.5 py-0.5 text-[9px] rounded bg-red-100 text-red-700">
                            {data.implantInventory.lowStock} low
                          </span>
                        )}
                      </h4>
                    </div>
                    <div className="space-y-1.5">
                      {data.implantInventory.categories.map((item) => (
                        <div key={item.name} className="flex items-center justify-between py-1 px-2 rounded bg-gray-50">
                          <span className="text-[10px] text-gray-700">{item.name}</span>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-gray-500">{item.inStock} units</span>
                            <span className={`text-[9px] font-medium ${getStockStatusClasses(item.status)}`}>
                  ...
                  <div className="p-3 rounded-lg bg-white border border-gray-200">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[10px] text-gray-500">
                        {data.physioSchedule.completed}/{data.physioSchedule.todaySessions} done
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      {data.physioSchedule.protocols.map((protocol) => (
                        <div key={protocol.name} className="p-2 rounded bg-teal-50 border border-teal-100">
                          <p className="text-[10px] font-medium text-teal-800 truncate">{protocol.name}</p>
                          <p className="text-xs font-bold text-teal-700">{protocol.activeCases} cases</p>
                        </div>
                      ))}
                    </div>
                  </div>

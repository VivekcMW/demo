...
import { oncologyDashboardData } from "@/data/seeded/oncology";

function getChemoStatusClasses(status: string): string {
  if (status === "completed") return "bg-green-100 text-green-700";
  if (status === "infusing") return "bg-blue-100 text-blue-700";
  return "bg-amber-100 text-amber-700";
}

const sidebarItems = [
  { icon: LayoutDashboard, label: "Dashboard", active: true },
  { icon: Users, label: "OPD Queue", badge: "18" },
  { icon: Pill, label: "Chemo Suite", badge: "24" },
  { icon: UsersRound, label: "Tumor Board", badge: "3" },
  { icon: Microscope, label: "Cancer Registry" },
  { icon: Calendar, label: "Follow-ups", badge: "32" },
...
export function OncologyDashboard() {
  ...
  const data = oncologyDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/oncology/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Oncology</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Priya Menon</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Medical Oncologist</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <Stat
                title="Chemo Cycles"
                value={data.todayStats.chemoCycles}
              ...
              <Stat
                title="Tumor Boards"
                value={data.todayStats.tumorBoards}
                icon={<UsersRound className="w-4 h-4 text-purple-500" />}
              />
              <Stat
                title="New Diagnosis"
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Chemotherapy Suite</h3>
                  <span className="text-[10px] text-pink-600 font-medium">{data.activeChemo.length} active</span>
                </div>
                <div className="space-y-2">
                  {data.activeChemo.map((patient) => (
                    <div key={patient.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-500">{patient.cancer} • {patient.regimen}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-gray-600 font-medium">Cycle {patient.cycle}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${getChemoStatusClasses(patient.status)}`}>
                          {patient.status}
                        </span>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Cancer Registry</h3>
                  <span className="text-[10px] text-purple-600 font-medium">{data.cancerRegistry.breast.total + data.cancerRegistry.lung.total + data.cancerRegistry.gi.total + data.cancerRegistry.hematological.total} patients</span>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-pink-50">
                    <span className="text-xs font-medium text-gray-700">Breast Cancer</span>
                    <span className="text-sm font-bold text-pink-700">{data.cancerRegistry.breast.total}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">Lung Cancer</span>
                    <span className="text-sm font-bold text-blue-700">{data.cancerRegistry.lung.total}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-amber-50">
                    <span className="text-xs font-medium text-gray-700">GI Cancers</span>
                    <span className="text-sm font-bold text-amber-700">{data.cancerRegistry.gi.total}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">Hematological</span>
                    <span className="text-sm font-bold text-purple-700">{data.cancerRegistry.hematological.total}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Today&apos;s Tumor Board</h3>
                <span className="text-[10px] text-purple-600 font-medium">{data.tumorBoardQueue.length} cases</span>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-100">
                      ...
                      <th className="text-left py-2 text-gray-500 font-medium">Cancer</th>
                      <th className="text-center py-2 text-gray-500 font-medium">Stage</th>
                      <th className="text-right py-2 text-gray-500 font-medium">Time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.tumorBoardQueue.map((patient) => (
                      <tr key={patient.id} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-2 font-medium text-gray-900">{patient.patient}</td>
                        <td className="py-2 text-gray-600">{patient.cancer}</td>
                        <td className="py-2 text-center">
                          <span className="px-2 py-0.5 rounded bg-red-100 text-red-700 text-[10px] font-medium">
                            {patient.stage}
                          </span>
                        </td>
                        <td className="py-2 text-right text-gray-500">{patient.time}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly Chemo & Consultations"
              data={data.weeklyTrend}
              primaryLabel="Chemo"
              secondaryLabel="Consults"
            />
          </div>
        </div>
      </div>

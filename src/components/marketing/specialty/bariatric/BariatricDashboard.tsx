...
import { bariatricDashboardData } from "@/data/seeded/bariatric";
...
const sidebarItems = [
  ...
  { icon: Scissors, label: "Surgery List", badge: "3" },
  { icon: Apple, label: "Nutrition", badge: "8" },
  { icon: Target, label: "Outcomes" },
  { icon: Activity, label: "Post-Op Registry" },
...
function Stat({ title, value, icon, trend, highlight }: StatProps) {
  return (
    <div className={`p-3 rounded-lg border ${highlight ? "bg-fuchsia-50 border-fuchsia-200" : "bg-white border-gray-200"}`}>
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] text-gray-500 uppercase tracking-wide">{title}</span>
        {icon}
      </div>
      <p className={`text-xl font-bold ${highlight ? "text-fuchsia-700" : "text-gray-900"}`}>{value}</p>
      {trend && (
...
export function BariatricDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = bariatricDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/bariatric/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Bariatric Surgery</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="h-11 bg-white border-b border-gray-200 px-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="md:hidden w-7 h-7 rounded-lg bg-fuchsia-600 flex items-center justify-center">
                <Scale className="w-4 h-4 text-white" />
              </div>
              <div className="relative hidden sm:block">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1 text-xs rounded-md border border-gray-200 bg-gray-50 w-44 focus:outline-none focus:ring-1 focus:ring-fuchsia-500 focus:border-fuchsia-500"
                />
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                <div className="w-6 h-6 rounded-full bg-fuchsia-600 flex items-center justify-center text-white text-[10px] font-semibold">
                  AJ
                </div>
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Amit Joshi</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Bariatric Surgeon</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <Stat
                ...
                icon={<Users className="w-4 h-4 text-fuchsia-500" />}
                highlight
              />
              <Stat
                title="Surgeries"
              ...
              <Stat
                title="Post-Op F/U"
                value={data.todayStats.postOpFollowUp}
                icon={<Calendar className="w-4 h-4 text-blue-500" />}
              />
              <Stat
                title="Nutrition"
                value={data.todayStats.nutritionConsult}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-fuchsia-600 font-medium">{data.surgerySchedule.length} cases</span>
              </div>
                ...
                {data.surgerySchedule.map((surgery) => (
                  <div key={surgery.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">{surgery.procedure} • BMI: {surgery.bmi}</p>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Outcomes (%EWL)</h3>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-green-50">
                    <span className="text-xs font-medium text-gray-700">6 Month Avg</span>
                    <span className="text-sm font-bold text-green-700">{data.outcomes.excessWeightLoss.avg6Month}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">12 Month Avg</span>
                    <span className="text-sm font-bold text-blue-700">{data.outcomes.excessWeightLoss.avg12Month}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">24 Month Avg</span>
                    <span className="text-sm font-bold text-purple-700">{data.outcomes.excessWeightLoss.avg24Month}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Comorbidity Resolution</h3>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 rounded-lg bg-red-50 text-center">
                    <p className="text-lg font-bold text-red-700">{data.outcomes.diabetesRemission}%</p>
                    <p className="text-[10px] text-gray-500">Diabetes</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.outcomes.hypertensionRemission}%</p>
                    <p className="text-[10px] text-gray-500">HTN</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.outcomes.osaImproved}%</p>
                    <p className="text-[10px] text-gray-500">OSA</p>
                  </div>
                </div>
              </div>
            </div>

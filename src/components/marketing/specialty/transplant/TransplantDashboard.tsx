...
import { transplantDashboardData } from "@/data/seeded/transplant";

function getStatusClasses(status: string): string {
  if (status === "in-surgery") return "bg-green-100 text-green-700";
  if (status === "in-eval") return "bg-purple-100 text-purple-700";
...
const sidebarItems = [
  ...
  { icon: Users, label: "Waitlist", badge: "124" },
  { icon: ClipboardCheck, label: "Evaluations", badge: "18" },
  { icon: HeartPulse, label: "Active Cases", badge: "8" },
  { icon: Stethoscope, label: "Post-Op" },
...
export function TransplantDashboard() {
  ...
  const data = transplantDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/transplant/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Transplant</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Sanjay Kumar</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Transplant Surgery</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <Stat
                title="Evaluations"
                value={data.todayStats.evaluations}
                icon={<ClipboardCheck className="w-4 h-4 text-cyan-500" />}
                highlight
              />
              <Stat
                title="Transplants"
                value={data.todayStats.transplants}
                icon={<Heart className="w-4 h-4 text-red-500" />}
              />
              <Stat
                title="Post-Op"
                value={data.todayStats.postOpRounds}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              ...
              <div className="space-y-2">
                {data.waitlistQueue.map((patient) => (
                  <div key={patient.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${patient.priority === "urgent" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
                        {patient.priority === "urgent" ? "URG" : "HIGH"}
                      </div>
                      <div>
                        <p className="text-xs font-medium text-gray-900">{patient.name}</p>
                        <p className="text-[10px] text-gray-500">{patient.organ} • Wait: {patient.waitTime}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Organ Waitlist</h3>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-red-50 text-center">
                    <p className="text-lg font-bold text-red-700">{data.waitlistStatus.kidney}</p>
                    <p className="text-[10px] text-gray-500">Kidney</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.waitlistStatus.liver}</p>
                    <p className="text-[10px] text-gray-500">Liver</p>
                  </div>
                  <div className="p-2 rounded-lg bg-cyan-50 text-center">
                    <p className="text-lg font-bold text-cyan-700">{data.waitlistStatus.heart}</p>
                    <p className="text-[10px] text-gray-500">Heart</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.waitlistStatus.lung}</p>
                    <p className="text-[10px] text-gray-500">Lung</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-green-50">
                    <span className="text-xs font-medium text-gray-700">1-Year Survival</span>
                    <span className="text-sm font-bold text-green-700">{data.outcomes.oneYearSurvival}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">Graft Function</span>
                    <span className="text-sm font-bold text-blue-700">{data.outcomes.graftFunction}%</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">Rejection Rate</span>
                    <span className="text-sm font-bold text-purple-700">{data.outcomes.rejectionRate}%</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly Evaluations & Transplants"
              data={data.weeklyTrend}
              primaryLabel="Evaluations"
              secondaryLabel="Transplants"
            />
          </div>
        </div>
      </div>

...
import { physiotherapyRehabDashboardData } from "@/data/seeded/physiotherapy-rehab";
...
const sidebarItems = [
  ...
  { icon: Users, label: "Sessions", badge: "86" },
  { icon: Target, label: "Assessments", badge: "12" },
  { icon: Waves, label: "Hydrotherapy" },
  { icon: Activity, label: "Outcomes" },
  { icon: FileText, label: "Home Plans" },
...
export function PhysiotherapyRehabDashboard() {
  const [activeNav, setActiveNav] = useState("Dashboard");
  const data = physiotherapyRehabDashboardData;
      ...
      <div className="bg-gray-200 px-4 py-2.5 flex items-center gap-3 border-b border-gray-300">
        ...
        <div className="flex-1 flex justify-center">
          <div className="bg-white rounded-md px-4 py-1.5 text-xs text-gray-600 flex items-center gap-2 min-w-70 sm:min-w-85">
            ...
            <span className="truncate">app.aarogyaehr.com/physiotherapy/dashboard</span>
          </div>
        </div>
        <div className="w-12" />
      </div>
          ...
          <div className="p-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400">Physiotherapy</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-gray-200">
                ...
                <div className="hidden sm:block">
                  <p className="text-xs font-medium text-gray-900 leading-tight">Dr. Kavitha Naidu</p>
                  <p className="text-[9px] text-gray-500 leading-tight">Physiotherapist</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <Stat
                ...
                value={data.todayStats.sessions}
                icon={<Activity className="w-4 h-4 text-emerald-500" />}
                highlight
              />
              <Stat
                title="Assessments"
                value={data.todayStats.newAssessments}
                icon={<Target className="w-4 h-4 text-blue-500" />}
              />
              <Stat
                title="Hydrotherapy"
                value={data.todayStats.hydrotherapy}
                icon={<Waves className="w-4 h-4 text-cyan-500" />}
              />
              <Stat
                title="Home Plans"
                value={data.todayStats.homePlans}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Session Queue</h3>
                <span className="text-[10px] text-emerald-600 font-medium">{data.sessionQueue.length} patients</span>
              </div>
              <div className="space-y-2">
                {data.sessionQueue.map((session) => (
                  <div key={session.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                    <div>
                      <p className="text-xs font-medium text-gray-900">{session.name}</p>
                      <p className="text-[10px] text-gray-500">{session.condition} • {session.session}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-500">{session.time}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${getStatusClasses(session.status)}`}>
                        {session.status}
                      </span>
                    </div>
                  </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-green-50 text-center">
                    <p className="text-lg font-bold text-green-700">{data.outcomeMetrics.painReduction}%</p>
                    <p className="text-[10px] text-gray-500">Pain Relief</p>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-50 text-center">
                    <p className="text-lg font-bold text-blue-700">{data.outcomeMetrics.romImprovement}%</p>
                    <p className="text-[10px] text-gray-500">ROM Improved</p>
                  </div>
                  <div className="p-2 rounded-lg bg-purple-50 text-center">
                    <p className="text-lg font-bold text-purple-700">{data.outcomeMetrics.functionalGain}%</p>
                    <p className="text-[10px] text-gray-500">Functional</p>
                  </div>
                  <div className="p-2 rounded-lg bg-emerald-50 text-center">
                    <p className="text-lg font-bold text-emerald-700">{data.outcomeMetrics.returnToWork}%</p>
                    <p className="text-[10px] text-gray-500">RTW</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">Active Patients</h3>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-50">
                    <span className="text-xs font-medium text-gray-700">Musculoskeletal</span>
                    <span className="text-sm font-bold text-emerald-700">{data.serviceBreakdown.musculoskeletal.active}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-purple-50">
                    <span className="text-xs font-medium text-gray-700">Neurological</span>
                    <span className="text-sm font-bold text-purple-700">{data.serviceBreakdown.neurological.active}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50">
                    <span className="text-xs font-medium text-gray-700">Sports</span>
                    <span className="text-sm font-bold text-blue-700">{data.serviceBreakdown.sports.active}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Weekly Trend */}
            <TrendChart
              title="Weekly Sessions & Assessments"

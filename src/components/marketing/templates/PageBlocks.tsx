    ...
    <section className="py-12 sm:py-16 md:py-20 lg:py-28 bg-gradient-to-b from-[var(--bg-subtle)] to-white">
      <Container>
        <div className="max-w-3xl mx-auto text-center px-2">
          ...
          <p className="text-base sm:text-lg md:text-xl text-[var(--text-secondary)] mb-6 sm:mb-8 leading-relaxed">
            {subtitle}
          </p>
      ...
      <Container>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          {features.map((feature, idx) => (
            <div
              key={feature.title}
              className="p-4 sm:p-6 rounded-lg sm:rounded-xl border border-[var(--border-default)] bg-white hover:shadow-lg transition-shadow"
            >
              {feature.icon && (
                <feature.icon className="w-8 h-8 sm:w-10 sm:h-10 text-[var(--action-primary)] mb-3 sm:mb-4" />
              )}
        ...
        <ul className="space-y-2 sm:space-y-3">
          {items.map((item, idx) => (
            <li key={idx} className="flex gap-2 sm:gap-3 items-start">
              <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--action-primary)] mt-0.5 flex-shrink-0" />
              <span className="text-sm sm:text-base text-[var(--text-secondary)]">{item}</span>
            </li>
          ))}
        </ul>
      </Container>
      ...
      <Container narrow>
        ...
        <div className="space-y-4 sm:space-y-6">
          {faqs.map((faq, idx) => (
            <div
              key={idx}
              className="p-4 sm:p-6 rounded-lg sm:rounded-xl bg-white border border-[var(--border-default)]"
            >
...
export function ProseContent({ content, className = "" }: ProseContentProps) {
  ...
  const processContent = (text: string) => {
    return text
      ...
      .replace(/\*(.+?)\*/g, "<em>$1</em>")
      .replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2" class="text-[var(--action-primary)] hover:underline">$1</a>')
      .replace(/\n\n/g, "</p><p>")
      .replace(/\n/g, "<br />");
  };
